...
if (isset($_GET['RoomReservID'])) {
	...
	if (empty($BorrowerName)) {
		header("Location: ../EditRReserv.php?RoomReservID=$RoomReservID&error=Borrower name is required");
            ...
            header("Location: ../EditRReserv.php?RoomReservID=$RoomReservID&error=Room ID is required");
        }
    else if (empty($RoomName)) {
            header("Location: ../EditRReserv.php?RoomReservID=$RoomReservID&error=Room Name is required");
    }
    else if (empty($RoomDetails)) {
            header("Location: ../EditRReserv.php?RoomReservID=$RoomReservID&error=Room Details is required");
    }
    else if (empty($reservDate)) {
            header("Location: ../EditRReserv.php?RoomReservID=$RoomReservID&error=Reservation Date is required");
    }
    else{
